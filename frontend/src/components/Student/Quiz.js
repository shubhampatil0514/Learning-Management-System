...
const Quiz = () => {
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        const response = await axios.get("http://localhost:8000/quize/subjects");
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/quize/questions/${selectedSubjectId}`);
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post("http://localhost:8000/quize/submit", {
    ...
    <>
      <div className="h-screen w-full bg-white font-sans">
        ...
        <button
          ...
          className="w-full bg-pink-600 text-white py-2 px-4 rounded-md mt-6 focus:outline-none focus:ring focus:ring-blue-200 focus:ring-opacity-50"
        >
          Submit Quiz
        </button>
        {showResults && (
